...
我們可以把 AI 信任拆成三個核心元素：
1. 可理解（Understandable）:  使用者能看懂 AI 為什麼這樣做，也就是 XAI （Explainability AI)的概念。 就像你去看醫生，醫生如果只丟了一句「吃這個藥」就可以了，你是否或抱有疑慮甚至不知道自己生了什麼病，但如果醫生有解釋「因為你的症狀 A、B、C，這些藥可以如何幫助你」，這樣你會更願意聽從醫囑吧。 
2. 可控（Controllable） : 使用者保有覆核、修正、甚至停用 AI 的權力（Human-in-the-loop）。比如你到餐廳點菜，菜單上有一道大大的「主廚推薦」，但你也可以選擇不點而改點其他的菜，甚至沒看到想吃的也可直接離開餐廳。 
3. 可預期（Predictable） : 使用者在同樣情境下看到一致的反應，模型要確保在同樣輸入下輸出穩定，並能對「不確定」情況給出信心分數。  以及風險管理和版本控管，避免 AI 模型在更新後行為大變，造成使用者無法預期。
...
## 二、XAI(Explainable AI) 是AI信任的核心，但要「以人為中心」
