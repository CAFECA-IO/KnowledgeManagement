...
### 假設我們有一個 EIP-1559 raw transaction sample//可以在ethereumscan中查找:
...
解析後得到 JSON object：

	{
	  ...
	  "nonce": "180", //  b4   
	  "gasLimit": "286441", // 45EE9
	  "maxFeePerGas": "9745321614", //244ddce8e
	  "maxPriorityFeePerGas": "10909998", //a6792e
	  ...
	  "value": "200000000000000000", // 2c68af0bb140000
   ...
   5. "nonce": 是發送者地址的計數，確保交易按正確的順序處理並防止雙重支付，此交易來說，這是使用者錢包的第180次交易 //180 = b4
	...
	1. 計算簽章證明的模逆，即簽章產生函數的逆：確定模數N：需要知道要在其下執行模逆計算的模數N。這是一個正整數。選擇要計算逆的數字a：要找到其逆的數字。通常，a 必須是模數N的正整數。計算逆元素x：要計算a的逆元素x，需要找到一個數字x，使得以下條件成立：(a * x) % N = 1。這表示a和x的乘積在模N下等於1。
 	2. 從 x 座標恢復簽名期間產生的隨機點r
  	3. 從R'產生其x座標：r' = R' .x
       ...
       Sveltin Nakov:"簽章簽章使用私鑰privKey和訊息雜湊h透過橢圓曲線變換將隨機點R （僅由其 x 座標表示）編碼為數位s，這是訊息簽署者知道私鑰的證明私鑰。由於ECDLP問題的難度，簽章{ r , s }無法洩漏私鑰。簽章驗證使用公鑰pubKey和訊息雜湊h將簽章中的證明編號s解碼回其原始點R ，並將恢復的R的 x 座標與簽章中的r值進行比較"
