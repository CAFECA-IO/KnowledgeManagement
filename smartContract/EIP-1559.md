...
RLP標準化了在節點之間以節省空間的方式傳輸數據。RLP的目的是將任意嵌套的二進制數據數組編碼，正整數的RLP必須以大端二進制形式表示，不帶前導零（從而使整數值零等同於空字節數組），帶前導零的反序列化正整數被視為無效。
#### Definition
...
4. 如果一個列表的總有效載荷（即其所有項目的組合長度，經過RLP編碼後）長度在0到55字節之間，那麼RLP編碼由以下部分組成：第一個字節的數值是0xc0加上列表的長度。接著是所有項目的RLP編碼的串聯。在這種情況下，第一個字節的範圍是 [0xc0, 0xf7]（十進制 [192, 247]）。
5. 如果一個列表的總有效載荷超過55字節，那麼RLP編碼由以下部分組成：第一個字節的數值是0xf7加上有效載荷長度的長度（即字節數的二進制表示的長度）。接著是有效載荷的長度。最後是所有項目的RLP編碼的串聯。在這種情況下，第一個字節的範圍是 [0xf8, 0xff]（十進制 [248, 255]）。
...
1. 如果前綴的範圍在 [0x00, 0x7f] 內，則該數據被視為字符串，並且該字符串就是第一個字節本身；

2. 如果第一個字節的範圍在 [0x80, 0xb7] 內，則該數據被視為字符串，並且該字符串的長度等於第一個字節減去 0x80，然後跟隨在第一個字節之後；

3.如果第一個字節的範圍在 [0xb8, 0xbf] 內，則該數據被視為字符串，並且字符串的長度（以字節為單位）等於第一個字節減去 0xb7，然後跟隨在第一個字節之後，接著是字符串的內容；

4.如果第一個字節的範圍在 [0xc0, 0xf7] 內，則該數據被視為列表，其中所有項目的總有效載荷等於第一個字節減去 0xc0，然後跟隨在第一個字節之後，接著是列表中所有項目的RLP編碼的串聯；

5.如果第一個字節的範圍在 [0xf8, 0xff] 內，則該數據被視為列表，列表的長度等於第一個字節減去 0xf7，然後跟隨在第一個字節之後，接著是列表的總有效載荷，最後是列表中所有項目的RLP編碼的串聯。
