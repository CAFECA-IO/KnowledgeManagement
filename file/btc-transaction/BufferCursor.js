// https://github.com/bmancini55/crypto-playpen/blob/master/bitcoin2/src/buffer-cursor.js
...
class BufferCursor {
  ...
  readBytes(len) {
    if (len === 0) {
      ...
      const result = this._buffer.slice(this._position, this._position + len);
      this._position += len;
      return result;
    }
    if (this._position === this._buffer.length) throw new RangeError('Index out of range');
    const result = this._buffer.slice(this._position);
  ...
  writeBytes(buffer) {
    ...
    if (this._position + buffer.length > this._buffer.length) { throw new RangeError('Index out of range'); }
