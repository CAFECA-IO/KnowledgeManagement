# 自動化測試研究紀錄
## Summary:
為了加速測試進行，本研究進行了針對 TideBit 可以採取的自動化介面測試研究。有關於自動化測試，Selenium、Katalon Studio、UFT 等都是測試時可能會使用的的工具，但考量到 Selenium 有方便使用、支援多種模式和程式語言（Java、Groovy、Python、C＃等）等特性，故選用 Selenium 作為研究主題。
