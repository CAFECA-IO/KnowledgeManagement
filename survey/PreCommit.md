...
為確保每次 commit 前的程式碼是乾淨、符合 coding style 且能通過測試的，我們需要經過 test、format、eslint 等環節。本指南旨在提供一個完整的 pre-commit 檢查流程，確保程式碼品質和一致性。
...
本指南針對 Typescript 做設置，區分為後端專案跟前端專案。前端專案以 Next.js 為例，後端專案以 Node.js 為例，但兩者有一些相通的環境設置。
