...
### 加入特定資料進行簽名
在使用FIDO2/WebAuthn標準進行用戶註冊時，可能可以通過**挑戰（Challenge）**來傳遞特定想要寫入的資料，並由用戶的設備進行簽名後送回服務端。然而，這些"特定資料"並不是直接作為信息的一部分儲存在挑戰本身，而是通過在生成挑戰時將這些資料納入挑戰的生成過程中來實現。

### 如何實現

當server端生成挑戰（Challenge）時，可以結合我們想要用戶簽名的特定資料，作為生成挑戰的一部分。這種方式通常涉及到server端的一些業務邏輯，例如：
